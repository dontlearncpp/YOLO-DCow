...
class Proto(nn.Module):
    ...
    def forward(self, x):
        ...
        # img = Image.open(
        #     "/media/xingshixu/367a0adf-4bec-4c64-b23d-945aacb28ba5/yangyolo/ultralytics-maincowfull-shuchu/xing.png").convert(
        #     'RGB')
        ...
        # heatmap = heat_mean.cpu().detach().numpy()  # 转换为numpy数组
        ...
        # f = np.fft.fft2(heatmap)
        # fshift = np.fft.fftshift(f)
        # mag = 40 * np.log(np.abs(fshift))
        # mag = mag.astype(np.uint8)
        # mag = cv2.applyColorMap(mag, cv2.COLORMAP_JET)  # 颜色变换
        # cv2.imwrite('./heatmapct-mag' + '.jpg', mag)
        #
        # heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)  # 颜色变换
        # plt.imshow(heatmap)
        # plt.show()
        # cv2.imwrite('./heatmapct' + '.jpg', heatmap)

        out=self.cv3(self.cv2(x))



        # img = Image.open(
        #     "/media/xingshixu/367a0adf-4bec-4c64-b23d-945aacb28ba5/yangyolo/ultralytics-maincowfull-shuchu/test.png").convert(
        #     'RGB')
        # height, width = img.size
        # heat = out.squeeze(0)  # 降维操作,尺寸变为(2048,7,7)
        # values = out
        # values = values.permute(1, 0, 2, 3)
        # image_name = 'result' + '.png'
        # torchvision.utils.save_image(values, image_name)
        ...
        # mag = mag.astype(np.uint8)
        # mag = cv2.applyColorMap(mag, cv2.COLORMAP_JET)  # 颜色变换
        # cv2.imwrite('./heatmapout-mag' + '.jpg', mag)
        #
        # heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)  # 颜色变换
        # plt.imshow(heatmap)
        # plt.show()
        # cv2.imwrite('./heatmapout' + '.jpg', heatmap)
