    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                depthpath = os.path.split(f)[0]
                if depthpath[-5:] == "train":
                    b = depthpath[:-5]
                    depth_path=b+"traindepth/"+os.path.split(f)[1]
                    imgOri = cv2.imread(depth_path, -1)
                else:
                    b = depthpath[:-3]
                    depth_path = b + "traindepth/" + os.path.split(f)[1]
                    imgOri = cv2.imread(depth_path, -1)
                # imgOri[imgOri > 10000] = 10000
                _range = np.max(imgOri) - np.min(imgOri)
                box = (imgOri - np.min(imgOri)) / _range
                depth = box * 255
